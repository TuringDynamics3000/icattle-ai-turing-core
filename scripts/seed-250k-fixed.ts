/**
 * FIXED 250k Cattle Database Seed Script
 * Uses proper transactions and parameterized queries
 */
...
function generateGPS(baseLat: number, baseLng: number, variance: number = 0.5) {
  return {
    lat: parseFloat((baseLat + (Math.random() - 0.5) * variance).toFixed(6)),
    lng: parseFloat((baseLng + (Math.random() - 0.5) * variance).toFixed(6)),
  };
}
...
async function seed() {
  console.log('🌱 Starting FIXED 250k cattle database seed...\n');
  ...
  try {
    ...
    // STEP 1: Truncate all tables in a transaction
    ...
    // STEP 4: Generate 250,000 cattle using COPY for maximum performance
    console.log('🐄 Generating 250,000 cattle records...');
    console.log('   This will take several minutes...\n');
    
    const BATCH_SIZE = 5000; // Larger batches with COPY
    ...
    for (let batch = 0; batch < TOTAL_CATTLE / BATCH_SIZE; batch++) {
      ...
      for (let i = 0; i < BATCH_SIZE; i++) {
        ...
        const valuation = weight * randomInt(350, 450);
        ...
        cattleBatch.push({
          ...
          current_location: `${farm.region} ${farm.state}`,
          latitude: gps.lat,
          longitude: gps.lng,
      ...
      // Insert batch using postgres.js insert (proper transaction handling)
      await sql`
    ...
    for (let i = 0; i < EVENTS_COUNT; i++) {
      ...
      if (eventBatch.length >= 1000 || i === EVENTS_COUNT - 1) {
        await sql`
          INSERT INTO lifecycle_events ${sql(eventBatch, 
    ...
    const totalValue = Number(stats[0].total_value) / 100;
